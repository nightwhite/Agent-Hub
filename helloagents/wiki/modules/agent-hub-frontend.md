...
- 文件工作台的目录浏览层现在还要求具备短时缓存与预取：同一路径的重复进入应优先命中本地缓存，目录项 hover 时可预取下一层 listing，刷新动作必须显式绕过缓存，文件写入/创建/删除/上传后要主动失效当前目录缓存
- 文件工作台的 UI 操作入口必须覆盖后端已支持的下载、删除、创建、上传与保存能力；删除必须带确认，避免容器内文件管理能力只存在于 hook 但无法被用户触发
